...
def admin_user(db):
    return User.objects.create_user(
        phone='+998900000109', password='pass1234',
...
class TestFinanceAuditTrail:
    ...
    def test_cancelling_transaction_logs_update(self, finance_user, admin_user, student):
        resp = auth_client(finance_user).post('/api/v1/finance/transactions/record/', {
            'student': str(student.id), 'month': 4, 'year': 2026, 'amount': 200000,
        })
        txn_id = resp.data['transaction']['id']
        cancel = auth_client(admin_user).post(
            f'/api/v1/finance/transactions/{txn_id}/cancel/', {'reason': 'Xato summa'})
        assert cancel.status_code == 200
        log = last_log(ActivityLog.Action.UPDATE, 'PaymentTransaction')
        assert log is not None
        assert log.user == admin_user
        assert str(log.object_id) == txn_id
        assert log.changes['cancel_reason']['new'] == 'Xato summa'

    def test_editing_payment_logs_update(self, admin_user, student, group):
        payment = Payment.objects.create(student=student, group=group, month=5, year=2026, amount=500000)
        client = auth_client(admin_user)
        resp = client.patch(f'/api/v1/payments/{payment.id}/', {'note': 'edited', 'discount': 200000})
        assert resp.status_code == 200
        log = last_log(ActivityLog.Action.UPDATE, 'Payment')
        assert log is not None
        assert log.user == admin_user
        assert log.changes['discount'] == {'old': '0', 'new': '200000'}
        assert log.changes['note']['new'] == 'edited'
        # debt_amount is derived by Payment.save() — the diff must reflect it.
...
@pytest.mark.django_db
class TestPaymentInvoiceAuditLabel:
    """POST /api/v1/payments/ endi faqat hisob ochadi: birinchi chaqiruv CREATE
    yozadi, takroriy chaqiruv hech narsani o'zgartirmaydi va jurnalga yozmaydi."""

    def test_first_call_logs_create(self, finance_user, student, group):
        resp = auth_client(finance_user).post('/api/v1/payments/', {
            'student': str(student.id), 'month': 9, 'year': 2026,
        })
        assert resp.status_code == 201
        assert last_log(ActivityLog.Action.CREATE, 'Payment') is not None
        assert last_log(ActivityLog.Action.UPDATE, 'Payment') is None

    def test_second_call_logs_nothing(self, finance_user, student, group):
        client = auth_client(finance_user)
        client.post('/api/v1/payments/', {'student': str(student.id), 'month': 10, 'year': 2026})
        count_before = ActivityLog.objects.filter(model_name='Payment').count()
        resp = client.post('/api/v1/payments/', {'student': str(student.id), 'month': 10, 'year': 2026})
        assert resp.status_code == 200
        assert ActivityLog.objects.filter(model_name='Payment').count() == count_before
...
class TestAuditChangesNoSensitiveData:
    ...
    def test_payment_update_changes_has_no_forbidden_keys(self, admin_user, student, group):
        payment = Payment.objects.create(student=student, group=group, month=11, year=2026, amount=500000)
        client = auth_client(admin_user)
        client.patch(f'/api/v1/payments/{payment.id}/', {'discount': 500000, 'note': 'grant'})
        log = last_log(ActivityLog.Action.UPDATE, 'Payment')
        assert log.changes
